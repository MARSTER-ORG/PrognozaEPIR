...
(()=>{
  ...
  const CLOUD_SHARE=.75,FEW_SCT_RULE_MIN_FT=4000;
  ...
  function patchArchive(){
    ...
    try{Object.defineProperty(W,'__tafPolicyV8',{value:true});window.PrognozaEPIRMessageArchive=W;archivePatched=true;return true}catch(_){return false}
  }
  ...
  function isFewSctPeriodCompatible(c,periodRuleActive){return periodRuleActive&&(c.amount==='FEW'||c.amount==='SCT')&&c.ft>=FEW_SCT_RULE_MIN_FT}
  function forecastHourState(tr,t,periodRuleActive){
    ...
    const conv=sig.some(x=>x.type),blocking=sig.filter(x=>!x.type&&!isFewSctPeriodCompatible(x,periodRuleActive));
  ...
  function forecastCloudEvidence(times){
    const periodRuleActive=isNightUtc(times.start),trs=[...document.querySelectorAll('#hours tr')].slice(0,12);if(trs.length<12)return null;
    const states=trs.map((tr,i)=>forecastHourState(tr,times.start+i*3600e3,periodRuleActive)).filter(Boolean);if(states.length!==12)return null;
    ...
    return{samples:states.length,compatible,share,cavokLike,night,periodRuleActive,supports:periodRuleActive&&share>CLOUD_SHARE,states};
  }
  ...
  // Gdy TAF rozpoczyna się między 18:00 a 05:00 UTC, oceniamy pełne 12 h.
  // Jeśli >75% godzin ma brak chmur istotnych albo tylko FEW/SCT >=4000 ft,
  // te warstwy FEW/SCT nie blokują kodu CAVOK/NSC w bazie.
  function applyForecastCloudRule(line,times,forecast){
    if(!forecast?.supports)return line;
    ...
    const conv=sig.some(c=>c.type),blocking=sig.filter(c=>!c.type&&!isFewSctPeriodCompatible(c,true));if(conv||blocking.length)return line;
  ...
  function mark(vrb,clear,forecast){
    disableSynopUi();const s=$('sources');if(s){
      ...
      let f=s.querySelector('[data-forecast-cavok]');if(!f){f=document.createElement('span');f.dataset.forecastCavok='1';s.appendChild(f)}f.className='pill '+(forecast?.supports?'ok':'');f.textContent=forecast?`12 h FEW/SCT>=4000 ${Math.round(forecast.share*100)}%${forecast.supports?' ✓':''}`:'12 h FEW/SCT>=4000 —';
    }
    const b=$('reasons');if(b){b.querySelectorAll('[data-taf-policy-reason]').forEach(x=>x.remove());let ul=b.querySelector('ul');if(!ul){ul=document.createElement('ul');b.appendChild(ul)}
      ...
      if(forecast){const li=document.createElement('li');li.dataset.tafPolicyReason='forecastcavok';li.textContent=`CAVOK/NSC: dla TAF rozpoczynającego się 18-05 UTC ${forecast.compatible}/${forecast.samples} godzin (${Math.round(forecast.share*100)}%) ma brak chmur istotnych lub tylko FEW/SCT >=4000 ft. Próg jest >75%.`;ul.appendChild(li)}
