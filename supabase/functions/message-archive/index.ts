...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(op==='latest'){
      ...
      const out:any={schema:'prognozaepir-message-archive-api-v4',updated_at:new Date().toISOString().replace('.000Z','Z'),taf_by_station:{}};for(const[t,s,r]of pairs){if(!r)continue;if(t==='METAR'){out.metar=r;out.metar_only=r}else if(t==='SPECI')out.speci=r;else if(t==='SYNOP')out.synop=r;else if(t==='TAF'){out.taf_by_station[s]=r;if(s==='EPIR')out.taf=r}}const aviation=[out.metar,out.speci].filter(Boolean).sort((a:any,b:any)=>Date.parse(a.message_time||0)-Date.parse(b.message_time||0)).pop();if(aviation)out.aviation=aviation;return json({ok:true,data:out});
    }
