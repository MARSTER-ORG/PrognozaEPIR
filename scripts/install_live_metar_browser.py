...
# One-shot installer used to patch the production browser observation engine.
# It is intentionally idempotent.

p = Path('taf.html')
s = p.read_text(encoding='utf-8')
if '<!-- deploy-compat: GENERATOR TAF v0.2.0 -->' not in s:
    if '<!-- deploy-compat: GENERATOR TAF v0.1.0 -->' in s:
        s = s.replace('<!-- deploy-compat: GENERATOR TAF v0.1.0 -->',
                      '<!-- deploy-compat: GENERATOR TAF v0.2.0 -->', 1)
    else:
        s = s.replace('<!-- TAF EPBY/EPPW/EPKS -->',
                      '<!-- TAF EPBY/EPPW/EPKS --><!-- deploy-compat: GENERATOR TAF v0.2.0 -->', 1)
p.write_text(s, encoding='utf-8')

p = Path('observation-engine.js')
s = p.read_text(encoding='utf-8')

if "const IMGW_METAR_URL" not in s:
    marker = "  const RECENT_URL = 'data/observations/recent.json';\n"
    assert marker in s, 'RECENT_URL marker missing'
    s = s.replace(marker, marker +
        "  const IMGW_METAR_URL = 'https://aviation-api.imgw.pl/data/last?params=metar,taf&format=json&count=4';\n", 1)

if 'async function fetchImgwLiveMetar' not in s:
    marker = '  function metarPhenomena(m){\n'
    assert marker in s, 'metarPhenomena marker missing'
    helper = r'''  function walkImgwMessages(value,out=[]){
...
old = r'''  async function bootstrap(){
    setVersion();
    try{
...
'''
new = r'''  async function refreshLiveMetar(){
    try{
...
'''
if 'setInterval(refreshLiveMetar,2*60*1000)' not in s:
    assert old in s, 'bootstrap block missing'
    s = s.replace(old, new, 1)

p.write_text(s, encoding='utf-8')
print('live IMGW browser patch ready')
