...
# Deterministic TAF production runtime: exactly one engine/app pair and one version.
t=OUT/'taf.html'; s=t.read_text(encoding='utf-8')
s=re.sub(r'<meta name="prognozaepir-taf-engine-v2" content="[^"]+">',f'<meta name="prognozaepir-taf-engine-v2" content="{TAF_VERSION}">',s,count=1)
s=re.sub(r'taf-engine-v2\.js(?:\?v=[^"\']*)?',f'taf-engine-v2.js?v={TAF_VERSION}-{SHA}',s)
s=re.sub(r'taf-app-v2\.js(?:\?v=[^"\']*)?',f'taf-app-v2.js?v={TAF_VERSION}-{SHA}',s)
...
print(f'Canonical Pages artifact ready: {len(list(OUT.iterdir()))} top-level items; TAF {TAF_VERSION} only; UI shell {SHA}')
