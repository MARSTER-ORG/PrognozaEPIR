#!/usr/bin/env python3
"""Build the canonical PrognozaEPIR GitHub Pages artifact.

The source repository remains the owner of application logic. This builder only:
- copies static application/data assets,
- applies the small compatibility hooks that are still not embedded in source HTML,
- wires the canonical TAF Engine 2.3 and active radar/lightning modules,
- rejects legacy TAF runtime references.
"""
from __future__ import annotations

import os
import re
import shutil
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
SITE = ROOT / "_site"
ASSET_V = os.environ.get("GITHUB_SHA", "dev")[:12]
APP = "v0.10.25 HTML"
RADAR_VERSION = "RADAR / SAT / AI v0.12.3"


def read(path: Path) -> str:
    return path.read_text(encoding="utf-8")


def write(path: Path, text: str) -> None:
    path.write_text(text, encoding="utf-8")


def copy_site() -> None:
    if SITE.exists():
        shutil.rmtree(SITE)
    SITE.mkdir(parents=True)

    # Root web assets. Copying by extension prevents the deploy list from
    # silently falling behind when a new active module/page is added.
    allowed = {".html", ".js", ".png", ".svg", ".webmanifest", ".ico"}
    for src in ROOT.iterdir():
        if src.is_file() and src.suffix.lower() in allowed:
            shutil.copy2(src, SITE / src.name)

    data = ROOT / "data"
    if data.exists():
        shutil.copytree(data, SITE / "data", dirs_exist_ok=True)


def ensure_once(text: str, snippet: str, before: str) -> str:
    if snippet in text:
        return text
    if before not in text:
        raise RuntimeError(f"site hook not found: {before[:80]}")
    return text.replace(before, snippet + "\n" + before, 1)


def remove_script(text: str, filename: str) -> str:
    return re.sub(
        rf"\s*<script\s+src=\"{re.escape(filename)}(?:\?[^\"]*)?\"></script>",
        "",
        text,
        flags=re.I,
    )


def patch_index() -> None:
    p = SITE / "index.html"
    s = read(p)

    favicon = (
        '<link rel="icon" href="epir-icon-192.png" type="image/png" sizes="192x192">\n'
        '<link rel="shortcut icon" href="epir-icon-192.png" type="image/png">\n'
        '<link rel="apple-touch-icon" href="epir-icon-192.png">'
    )
    if 'href="epir-icon-192.png"' not in s:
        s = s.replace("<title>", favicon + "\n<title>", 1)

    s = re.sub(r"v0\.\d+\.\d+ HTML", APP, s)
    s = re.sub(r"HTML v0\.\d+\.\d+", "HTML v0.10.25", s)
    s = s.replace(
        "function timeline(){const now=Date.now(),t0=Math.ceil(now/3600e3)*3600e3",
        "function timeline(){const now=Date.now(),t0=Math.floor(now/3600e3)*3600e3",
    )

    icm = '<a class="control-link" href="https://www.meteo.pl/um/php/meteorogram_id_um.php?ntype=0n&amp;id=2077" target="_blank" rel="noopener noreferrer">ICM UM meteogram ↗</a>'
    links = [
        '<a class="control-link" href="radar.html">RADAR / SAT / AI</a>',
        '<a class="control-link" href="taf.html?v=2.3.0">GENERATOR TAF</a>',
        '<a class="control-link" href="sat-fog.html">SAT / FOG EUMETSAT</a>',
    ]
    if icm in s:
        for link in reversed(links):
            if link not in s:
                s = s.replace(icm, link + "\n    " + icm, 1)

    s = s.replace(
        "{id:'icon_global',name:'ICON',w:.05,provider:'openmeteo'},",
        "{id:'icon_global',name:'ICON',w:.05,provider:'openmeteo',surfaceExclude:['visibility']},",
    )
    s = s.replace(
        "const vars=prof?profileVarsFor(model):SURFACE;",
        "const vars=prof?profileVarsFor(model):SURFACE.filter(v=>!(model.surfaceExclude||[]).includes(v));",
    )
    s = s.replace(
        "const msg=j?.error||j?.reason||j?.message||('HTTP '+r.status);",
        "const msg=j?.reason||j?.message||(typeof j?.error==='string'?j.error:('HTTP '+r.status));",
    )

    cloud_loader = '<script src="cloud-learning-client.js"></script>'
    if cloud_loader not in s:
        marker = "<script>\n'use strict';"
        if marker not in s:
            raise RuntimeError("index inline script marker missing")
        s = s.replace(marker, cloud_loader + "\n" + marker, 1)

    old_cloud = "const pr=profile(items,terr),ceil=ceiling(pr)"
    new_cloud = "const cloudItems=items.map(i=>({...i,w:i.w*(window.PrognozaEPIRCloudLearning?.factor?.(i.model.id,t)||1)})),cloudSw=cloudItems.reduce((a,b)=>a+b.w,0);if(cloudSw)cloudItems.forEach(i=>i.w/=cloudSw);const pr=profile(cloudItems,terr),ceil=ceiling(pr)"
    if new_cloud not in s:
        if old_cloud not in s:
            raise RuntimeError("Cloud Learning profile hook not found")
        s = s.replace(old_cloud, new_cloud, 1)

    old_load = "async function load(){setBadge('ŁADOWANIE','');failures=[];datasets.clear();demo=false;"
    new_load = "async function load(){try{await window.PrognozaEPIRCloudLearning?.loadSkill?.()}catch(_){ }setBadge('ŁADOWANIE','');failures=[];datasets.clear();demo=false;"
    if new_load not in s:
        if old_load not in s:
            raise RuntimeError("Cloud Learning load hook not found")
        s = s.replace(old_load, new_load, 1)

    # The observation engine is the single owner of loading FOG Engine. Remove
    # direct/old references first so the FOG runtime cannot execute twice.
    for name in (
        "fog-engine.js",
        "observation-engine.js",
        "mifg-engine.js",
        "fog-meteogram-overlay.js",
        "fog-summary-layout.js",
        "index-fixes.js",
        "axis-layout-fix.js",
        "visual-style-fix.js",
        "meteogram-layout-v2.js",
        "meteogram-visfog-split.js",
        "meteogram-visfog-cleanup.js",
        "rh-axis-fix.js",
        "day-night-fix.js",
        "shortcut-mode.js",
    ):
        s = remove_script(s, name)

    addons = [
        '<script src="index-fixes.js"></script>',
        '<script src="axis-layout-fix.js"></script>',
        '<script src="visual-style-fix.js"></script>',
        '<script src="meteogram-layout-v2.js"></script>',
        '<script src="meteogram-visfog-split.js"></script>',
        '<script src="rh-axis-fix.js"></script>',
        '<script src="meteogram-visfog-cleanup.js"></script>',
        '<script src="day-night-fix.js"></script>',
        '<script src="observation-engine.js"></script>',
        f'<script src="mifg-engine.js?v={ASSET_V}"></script>',
        '<script src="fog-meteogram-overlay.js"></script>',
        '<script src="shortcut-mode.js"></script>',
    ]
    s = s.replace("</body>", "\n".join(addons) + "\n</body>", 1)
    write(p, s)

    # Normalize app version and preserve the FOG series event contract in copied modules.
    for name in ("meteogram-layout-v2.js", "observation-engine.js", "fog-engine.js"):
        q = SITE / name
        if not q.exists():
            continue
        x = read(q)
        x = re.sub(r"const VERSION = 'v0\.\d+\.\d+ HTML';", f"const VERSION = '{APP}';", x)
        x = re.sub(r"const APP_VERSION = 'v0\.\d+\.\d+ HTML';", f"const APP_VERSION = '{APP}';", x)
        if name == "fog-engine.js" and "PrognozaEPIRFogSeries" not in x:
            old = "fogSeries=out;renderFog();"
            new = "fogSeries=out;window.PrognozaEPIRFogSeries=fogSeries;window.dispatchEvent(new CustomEvent('prognozaepir:fog-series-updated',{detail:{count:fogSeries.length}}));renderFog();"
            if old not in x:
                raise RuntimeError("Fog Engine series export hook not found")
            x = x.replace(old, new, 1)
        write(q, x)


def patch_taf() -> None:
    p = SITE / "taf.html"
    s = read(p)
    marker = '<meta name="prognozaepir-taf-engine-v2" content="2.3.0">'
    if "prognozaepir-taf-engine-v2" not in s:
        s = s.replace('<meta name="color-scheme" content="dark">', '<meta name="color-scheme" content="dark">\n  ' + marker, 1)
    else:
        s = re.sub(r'<meta name="prognozaepir-taf-engine-v2" content="[^"]+">', marker, s, count=1)
    s = re.sub(r'src="index\.html(?:\?v=[^"]*)?"', f'src="index.html?v={ASSET_V}"', s, count=1)
    s = re.sub(r'taf-engine-v2\.js\?v=[^"\']+', 'taf-engine-v2.js?v=2.3.0', s)
    s = re.sub(r'taf-app-v2\.js\?v=[^"\']+', 'taf-app-v2.js?v=2.3.0', s)
    write(p, s)


def patch_radar() -> None:
    p = SITE / "radar.html"
    s = read(p)
    favicon = (
        '<link rel="icon" href="epir-icon-192.png" type="image/png" sizes="192x192">\n'
        '<link rel="shortcut icon" href="epir-icon-192.png" type="image/png">\n'
        '<link rel="apple-touch-icon" href="epir-icon-192.png">'
    )
    if 'href="epir-icon-192.png"' not in s:
        s = s.replace("<title>", favicon + "\n<title>", 1)
    s = re.sub(r"RADAR / SAT / AI v\d+\.\d+\.\d+", RADAR_VERSION, s)
    s = s.replace(
        "function setDot(id,ok){$(id).className='dot '+(ok?'ok':'bad')}",
        "function setDot(id,ok){const el=$(id);if(el)el.className='dot '+(ok?'ok':'bad')}",
    )
    brand_old = '<div class="brand">PrognozaEPIR <small>'
    brand_new = '<div class="brand"><a href="index.html" title="Przejdź do meteogramu" aria-label="PrognozaEPIR — meteogram" style="color:inherit;text-decoration:none;cursor:pointer">PrognozaEPIR</a> <small>'
    if brand_old in s:
        s = s.replace(brand_old, brand_new, 1)

    meteo_link = '<a href="index.html">← Meteogram</a>'
    alert_link = '<a href="lightning-alerts.html">Alarm wyładowań</a>'
    if alert_link not in s and meteo_link in s:
        s = s.replace(meteo_link, alert_link + "\n    " + meteo_link, 1)

    radar_addon_names = [
        "radar-enhance.js",
        "warnings-readable.js",
        "radar-intelligence.js",
        "echo-analysis.js",
        "echo-fix.js",
        "lightning-layer.js",
        "ui-cleanup.js",
        "map-extras.js",
        "analysis-plus.js",
        "range-rings-reset.js",
        "opera-nowcast.js",
        "opera-convection-bridge.js",
        "radar-risk-policy.js",
        "shortcut-mode.js",
    ]
    for name in radar_addon_names:
        s = remove_script(s, name)

    radar_addons = [
        '<script src="radar-enhance.js"></script>',
        '<script src="warnings-readable.js"></script>',
        '<script src="radar-intelligence.js"></script>',
        '<script src="echo-analysis.js"></script>',
        '<script src="echo-fix.js"></script>',
        '<script src="lightning-layer.js"></script>',
        '<script src="ui-cleanup.js"></script>',
        '<script src="map-extras.js"></script>',
        '<script src="analysis-plus.js"></script>',
        '<script src="range-rings-reset.js"></script>',
        f'<script src="opera-nowcast.js?v={ASSET_V}"></script>',
        f'<script src="opera-convection-bridge.js?v={ASSET_V}"></script>',
        f'<script src="radar-risk-policy.js?v={ASSET_V}"></script>',
        '<script src="shortcut-mode.js"></script>',
    ]
    s = s.replace("</body>", "\n".join(radar_addons) + "\n</body>", 1)
    write(p, s)

    for name in (
        "radar-enhance.js",
        "radar-intelligence.js",
        "echo-analysis.js",
        "echo-fix.js",
        "lightning-layer.js",
        "map-extras.js",
        "analysis-plus.js",
        "range-rings-reset.js",
    ):
        q = SITE / name
        if not q.exists():
            continue
        x = re.sub(r"RADAR / SAT / AI v\d+\.\d+\.\d+", RADAR_VERSION, read(q))
        write(q, x)

    q = SITE / "radar-enhance.js"
    if q.exists():
        x = read(q)
        x = x.replace("    setTimeout(loadPolradProducts, 150);\n", "")
        start = x.find("  // --- Official IMGW / POLRAD product API ---")
        end = x.rfind("  const sources = document.querySelector('.sources');")
        if start >= 0 and end > start:
            x = x[:start] + x[end:]
        write(q, x)


def validate() -> None:
    required = [
        "index.html",
        "radar.html",
        "taf.html",
        "sat-fog.html",
        "arch.html",
        "taf-engine-v2.js",
        "taf-app-v2.js",
        "message-archive-client.js",
        "fog-engine.js",
        "observation-engine.js",
        "mifg-engine.js",
        "radar-risk-policy.js",
        "lightning-alerts.html",
        "lightning-alert-sw.js",
    ]
    missing = [name for name in required if not (SITE / name).is_file()]
    if missing:
        raise RuntimeError(f"missing site assets: {missing}")

    taf = read(SITE / "taf.html")
    for legacy in (
        "taf-hybrid-engine.js",
        "taf-hybrid-adapter.js",
        "taf-generator-policy.js",
        "taf-instruction-guard.js",
        "taf-output-sanitizer.js",
    ):
        if legacy in taf:
            raise RuntimeError(f"legacy TAF runtime referenced by deployed taf.html: {legacy}")
    for marker in (
        "TAF ENGINE 2.3.0 · INSTRUCTION FIRST",
        "taf-engine-v2.js?v=2.3.0",
        "taf-app-v2.js?v=2.3.0",
        "prognozaepir-taf-engine-v2",
    ):
        if marker not in taf:
            raise RuntimeError(f"missing TAF 2.3 marker: {marker}")

    index = read(SITE / "index.html")
    if "fog-summary-layout.js" in index:
        raise RuntimeError("dead fog-summary-layout.js reference still deployed")
    if re.search(r'<script\s+src="fog-engine\.js', index, re.I):
        raise RuntimeError("FOG Engine must be loaded only by observation-engine.js")
    if index.count("observation-engine.js") != 1:
        raise RuntimeError("observation-engine.js must be wired exactly once")

    radar = read(SITE / "radar.html")
    for marker in ("radar-risk-policy.js", "lightning-alerts.html"):
        if marker not in radar:
            raise RuntimeError(f"radar deployment missing {marker}")


def main() -> int:
    copy_site()
    patch_index()
    patch_taf()
    patch_radar()
    validate()
    print(f"Prepared canonical Pages artifact in {SITE}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
