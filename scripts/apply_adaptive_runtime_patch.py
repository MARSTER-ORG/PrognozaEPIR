...
def patch_index():
    ...
    s = replace_required(
        ...
        "function wavg(items,key){let s=0,w=0;for(const i of items){const v=i.row[key];if(finite(v)){const af=window.PrognozaEPIRAdaptiveWeights?.relativeFactor?.(i.model.id,i.t,key,i.context)||1,ww=i.w*af;s+=v*ww;w+=ww}}return w?s/w:null}",
        'adaptive wavg')
    s = replace_required(
        s,
        ...
        "function wind(items){let u=0,v=0,w=0;for(const i of items){const s=i.row.wind_speed_10m,d=i.row.wind_direction_10m;if(finite(s)&&finite(d)){const af=window.PrognozaEPIRAdaptiveWeights?.relativeFactor?.(i.model.id,i.t,'wind_speed_10m',i.context)||1,ww=i.w*af,z=uv(s,d);u+=z.u*ww;v+=z.v*ww;w+=ww}}return w?sd(u/w,v/w):{speed:null,dir:null}}",
        'adaptive wind')
    s = replace_required(
        s,
