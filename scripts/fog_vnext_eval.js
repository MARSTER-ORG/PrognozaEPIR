...
function main(){
  ...
  const out=rows.map(input=>{
    ...
    return {
      ...
      // Probability-valued verification fields have one explicit contract: 0..1.
      // Display-oriented physical family fields above remain percentages.
      physics_score:p.P_physics,
      direct_score:p.P_direct,
      model_final_shadow:p.P_model_final_shadow,
      ...
      surfaceCooling:Number.isFinite(v.surfaceCooling)?v.surfaceCooling:null,
      deltaSurfaceCooling1:Number.isFinite(v.deltaSurfaceCooling1)?v.deltaSurfaceCooling1:null,
      deltaSurfaceCooling3:Number.isFinite(v.deltaSurfaceCooling3)?v.deltaSurfaceCooling3:null,
      deltaTsurface3:Number.isFinite(v.deltaTsurface3)?v.deltaTsurface3:null,
      SATURATION:Number.isFinite(v.SATURATION)?v.SATURATION*100:null,
      MIXING:Number.isFinite(v.MIXING)?v.MIXING*100:null,
      SKY_COOLING:Number.isFinite(v.SKY_COOLING)?v.SKY_COOLING*100:null,
      dissipation:Number.isFinite(v.dissipationRisk)?v.dissipationRisk:null,
      observed_state_shadow:tr.state,
      onset_risk_shadow:Number.isFinite(tr.onsetRiskShadow)?tr.onsetRiskShadow:null,
      onset_historical_prior:Number.isFinite(tr.onsetHistoricalPrior)?tr.onsetHistoricalPrior:null,
      onset_confidence:Number.isFinite(tr.onsetConfidence)?tr.onsetConfidence:null,
      onset_window_shadow:tr.onsetWindow,
      dissipation_risk_shadow:Number.isFinite(tr.dissipationRiskShadow)?tr.dissipationRiskShadow:null,
      dissipation_historical_prior:Number.isFinite(tr.dissipationHistoricalPrior)?tr.dissipationHistoricalPrior:null,
