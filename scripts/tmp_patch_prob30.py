...
rep("""    const oldWx=cleanWx(weatherToken(prev,.5)),newWx=cleanWx(weatherToken(target,.5));
...
rep("""  function alternativeFogState(prev,target){
...
rep("    const groups=[],reasons=[],skipped=[];let prevailing=base,prob30Used=false;",
    "    const groups=[],reasons=[],skipped=[];let prevailing=base,prob30Used=false,prob30TempoUsed=false;")

rep("""      const newFog=targetFog!=='NONE'&&targetFog!==prevFog;
...
rep("      if(kind==='PROB30')prob30Used=true;",
    "      if(kind==='PROB30')prob30Used=true;else if(kind==='PROB30 TEMPO')prob30TempoUsed=true;")

rep("""    const prob30Count=(text.match(/\\bPROB30\\b/g)||[]).length;if(prob30Count>RULES.maxProb30Groups)errors.push(`PROB30 może wystąpić maksymalnie ${RULES.maxProb30Groups} raz w jednym TAF (polityka operacyjna EPIR).`);
...
t=Path('tests/taf-engine-v2.test.js')
q=t.read_text()
def trep(old,new):
    global q
    if old not in q:
        raise SystemExit('missing test marker: '+old[:120])
    q=q.replace(old,new,1)

trep("assert.equal(E.RULES.prob40,false);assert.equal(E.RULES.verticalVisibility,false);assert.equal(E.RULES.maxChangeGroups,5);assert.equal(E.RULES.maxProb30Groups,1);",
...
marker="assert.equal(E.validateTaf('TAF EPIR 132300Z 1400/1412 22004KT 7000 NSC PROB30 1404/1406 0900 FG PROB30 1406/1408 0900 FG=',{issue,start,end}).ok,false);\n"
addition=marker+"""assert.equal(E.validateTaf('TAF EPIR 132300Z 1400/1412 22004KT CAVOK PROB30 1404/1406 0900 FG PROB30 TEMPO 1407/1409 26018G28KT=',{issue,start,end}).ok,true);
