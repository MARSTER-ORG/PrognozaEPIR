...
def ingest(rows):
    ...
    for r in rows:
        if r: groups[dayfile(r)].append(r)
...
def legacy(full,speci):
    ...
    nb=js(ROOT/'data/taf/neighbors.json',{})
    for station,r in (nb.get('stations') or {}).items():
        if not isinstance(r,dict) or not r.get('raw'): continue
        r=dict(r); r['station']=station; r.setdefault('issue_time',iso(taf_time(r['raw'],dt(r.get('updated_at')) or now())))
        n=norm(r,'TAF','data/taf/neighbors.json'); out += [n] if n else []
    return out
...
def views(stat,full=False):
    since=now()-timedelta(hours=40); m=[x for x in stream('METAR',since) if x.get('station')=='EPIR']; sp=[x for x in stream('SPECI',since) if x.get('station')=='EPIR']; sy=[x for x in stream('SYNOP',since) if x.get('station')=='12342']; tf=stream('TAF',now()-timedelta(hours=72)); av=sorted(m+sp,key=lambda x:x['message_time'])
    tb={}
    for x in tf: tb[x.get('station')]=x
    latest={'schema':'prognozaepir-message-archive-latest-v1','archive':'data/messages','station':{'icao':'EPIR','synop':'12342','wigos':'0-20000-0-12342','lat':52.83,'lon':18.33},'metar':av[-1] if av else None,'metar_only':m[-1] if m else None,'speci':sp[-1] if sp else None,'aviation':av[-1] if av else None,'synop':sy[-1] if sy else None,'taf':tb.get('EPIR'),'taf_by_station':tb}
    latest['updated_at']=max([x['message_time'] for x in (latest['aviation'],latest['synop'],latest['taf']) if x],default=None)
    try: latest['fused']=obs.fuse(latest['aviation'],latest['synop'])
    except: latest['fused']=None
    try: hist=obs.history(av,sy)
    except: hist=[]
    recent={'schema':'prognozaepir-message-archive-recent-v1','archive':'data/messages','hours':40,'station':latest['station'],'metar':av,'metar_only':m,'speci':sp,'aviation':av,'synop':sy,'taf':tf,'observations':hist}
    writej(A/'latest.json',latest); writej(A/'recent.json',recent,False)
    prev=js(A/'status.json',{})
    if full or not prev.get('counts'):
        counts={k.lower():len(stream(k)) for k in TYPES}; last={k.lower():(stream(k)[-1]['message_time'] if stream(k) else None) for k in TYPES}
    else:
        counts={k.lower():int((prev.get('counts') or {}).get(k.lower(),0))+int(stat['added_by_type'].get(k.lower(),0)) for k in TYPES}; last=dict(prev.get('latest') or {})
        for k,row in {'metar':latest['metar_only'],'speci':latest['speci'],'synop':latest['synop'],'taf':tf[-1] if tf else None}.items():
            if row and (not last.get(k) or row['message_time']>last[k]): last[k]=row['message_time']
    writej(A/'status.json',{'schema':'prognozaepir-message-archive-status-v1','archive':'data/messages','types':list(TYPES),'counts':counts,'latest':last,'archive_updated_at':latest['updated_at'],'deduplication':'sha256(type + station + canonical_raw)','legacy_archives_authoritative':False})
    return counts
...
def main():
    ...
    writej(STATE,state); counts=views(st,full); checked=validate(); print(json.dumps({'full_migration':full,'legacy_seen':len(leg),'manual':ms,'ingest':st,'counts':counts,'checked':checked},ensure_ascii=False,indent=2))
