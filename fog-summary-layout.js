...
(() => {
  ...
  function loadPhysics(){
    ...
    return new Promise((resolve,reject)=>{
      ...
      const s=document.createElement('script');s.src='fog-physics-vnext.js?v=20260915-2';s.dataset.fogPhysicsVnext='1';
  ...
  async function fetchIcon(){
    ...
    let err=null;for(const vars of tiers)try{return rowsFrom(await fetchJson(apiUrl('icon_d2',vars)),'icon');}catch(e){err=e;}throw err||new Error('ICON-D2 vNext unavailable');
  }
  ...
  function t5cmObservation(t){
    ...
    const one=window.PrognozaEPIRT5cmObs;const candidates=one?[...rows,one]:rows;
    let best=null,bd=Infinity;for(const r of candidates){const rt=Date.parse(r?.obs_time||r?.time||r?.timestamp||'');const v=num(r?.temperature_c??r?.T5cm_obs);if(!finite(rt)||!finite(v))continue;const d=Math.abs(rt-t);if(d<bd){bd=d;best={temperature_c:v,obs_time:new Date(rt).toISOString(),source:r?.source||'T5cm_obs',ageHours:Math.abs(Date.now()-rt)/HOUR};}}
  ...
  function enrichSeries(Physics){
    ...
    for(const h of series){if(!finite(h?.t))continue;const input=inputFor(series,h),enhanced=Physics.enhanceLegacyHour(h,input);Object.assign(h,enhanced,{vnextInput:{tsurface:input.tsurface,soilTemperature0:input.soilTemperature0,soilTemperature6:input.soilTemperature6,soilTemperatureEcmwf07:input.soilTemperatureEcmwf07,t5cmObs:input.t5cmObs,t5cmObsTime:input.t5cmObsTime,t5cmObsSource:input.t5cmObsSource,t5cmObsAgeHours:input.t5cmObsAgeHours}});}
  ...
  function hourlyTable(series){
    const now=Date.now(),rows=series.filter(x=>x.t>=now-HOUR&&x.t<=now+12*HOUR).slice(0,13);
    if(!rows.length)return'';
    return `<div style="overflow-x:auto;margin-top:7px"><table style="border-collapse:collapse;width:100%;font-size:8px"><thead><tr><th>UTC</th><th>Soil/src</th><th>PBL</th><th>Δ1/Δ3</th><th>Tsurf</th><th>T2−Ts</th><th>SSOIL</th><th>SPBL</th><th>SSFC</th><th>Mech 1/2</th><th>Score</th><th>CONF</th><th>n</th></tr></thead><tbody>${rows.map(h=>{const v=h.vnext||{},vi=h.vnextInput||{},soil=finite(v.soilMoistureRaw)?fmt(v.soilMoistureRaw,v.soilMoistureSource==='precip proxy'?2:3):'—';return `<tr><td>${esc(localHour(h.t))}</td><td>${soil}<br>${esc(v.soilMoistureSource||'brak')}</td><td>${finite(v.PBL)?fmt0(v.PBL):'—'}</td><td>${fmt0(v.deltaPbl1)}/${fmt0(v.deltaPbl3)}</td><td>${finite(vi.tsurface)?fmt(vi.tsurface,1):'—'}</td><td>${finite(v.surfaceCooling)?fmt(v.surfaceCooling,1):'—'}</td><td>${fmt0(h.SSOIL)}</td><td>${fmt0(h.SPBL)}</td><td>${fmt0(h.SSFC_COOL)}</td><td>${esc(h.mechanism1||h.type?.primary||'—')}/${esc(h.mechanism2||h.type?.secondary||'—')}</td><td>${fmt0(h.score)}</td><td>${fmt0((h.confidence||0)*100)}%</td><td>${h.models?.length??0}</td></tr>`;}).join('')}</tbody></table></div>`;
  }
  function renderDiagnostics(){
    ...
    const soilRaw=finite(v.soilMoistureRaw)?(v.soilMoistureSource==='precip proxy'?`${fmt(v.soilMoistureRaw,2)} mm/12h`:`${fmt(v.soilMoistureRaw,3)} m³/m³`):'—',vi=h.vnextInput||{};
    const t5=finite(vi.t5cmObs)?`${fmt(vi.t5cmObs,1)} °C`:'—',t5sub=finite(vi.t5cmObsAgeHours)?`${vi.t5cmObsSource||'T5cm_obs'} · ${fmt(vi.t5cmObsAgeHours,1)} h`:'hook gotowy, brak źródła';
    details.innerHTML=`<summary>Fog Engine vNext — fizyka (SHADOW, nie steruje TAF)</summary><div class="fog-data-note"><b>Tryb:</b> shadow. Wynik operacyjny legacy pozostaje bez zmian do forecast→truth backtestu nowych pól. T5 cm jest wyłącznie opcjonalnym wejściem obserwacyjnym 0–3 h.</div><div class="fog-diag-grid">
      ...
      ${cell('Tsurface',finite(vi.tsurface)?fmt(vi.tsurface,1)+' °C':'—')}${cell('T2m − Tsurface',finite(v.surfaceCooling)?fmt(v.surfaceCooling,1)+' °C':'—')}${cell('Trend Tsurface 3 h',finite(v.deltaTsurface3)?(v.deltaTsurface3>=0?'+':'')+fmt(v.deltaTsurface3,1)+' °C':'—')}${cell('SSFC_COOL',fmt0(h.SSFC_COOL)+'/100')}${cell('Soil T 0 cm',finite(vi.soilTemperature0)?fmt(vi.soilTemperature0,1)+' °C':'—')}${cell('Soil T 6 cm',finite(vi.soilTemperature6)?fmt(vi.soilTemperature6,1)+' °C':'—')}${cell('T5 cm OBS',t5,t5sub)}
      ...
      ${cell('Mechanizm główny',h.mechanism1||h.type?.primary||'—')}${cell('Mechanizm drugi',h.mechanism2||h.type?.secondary||'—')}${cell('Fog score legacy',finite(h.score)?fmt0(h.score)+'/100':'—')}${cell('Fog score vNext shadow',finite(h.fogScoreVNextShadow)?fmt0(h.fogScoreVNextShadow)+'/100':'—')}${cell('Confidence',fmt0((h.confidence||0)*100)+'%')}${cell('Modele',String(h.models?.length??0))}${cell('Data quality vNext',fmt0((v.dataQuality||0)*100)+'%')}${cell('Braki',v.missing?.length?v.missing.join(', '):'brak')}${cell('Fallbacki',v.fallbacks?.length?v.fallbacks.join(', '):'brak')}
    </div>${hourlyTable(window.PrognozaEPIRFogSeries||[])}`;
  }
  ...
  window.addEventListener('prognozaepir:fog-series-updated',refresh);window.addEventListener('prognozaepir:fog-vnext-updated',renderDiagnostics);window.addEventListener('prognozaepir:t5cm-updated',()=>{lastAppliedSignature='';refresh();});setTimeout(refresh,200);
