...
(() => {
  ...
  function isWind(tok) {
    return /^(?:\d{3}|VRB)(?:P?\d{2,3})(?:G(?:P?\d{2,3}))?KT$/.test(tok) || tok === '00000KT';
  }
  ...
  function splitLine(line) {
    ...
    if (change) {
      const kind = change[1].trim().split(/\s+/)[0] === 'PROB30' ? (change[1].includes('TEMPO') ? 'PROB30 TEMPO' : 'PROB30') : change[1].trim().split(/\s+/)[0];
  ...
  function normalizeBody(body, ctx) {
    ...
    if (!original.length) return {body:'', drop:false, meta:{}};
    ...
    if (hasSigCloud) {
      ...
      // Brak istotnych chmur i brak zjawiska. CAVOK wolno utworzyć tylko,
      // gdy widzialność 10 km+ jest jawna albo wynika z obowiązującego CAVOK
      // i ta grupa rzeczywiście modyfikuje część VIS/WX/cloud.
      const canCavok = hadCavok || had9999 || (ctx.prevVis9999 && (hadNsw || hadNsc));
      if (canCavok && (ctx.base || hadMetPayload)) {
        tokens = tokens.filter(t => t !== 'CAVOK' && t !== '9999' && t !== 'NSW' && t !== 'NSC');
        tokens.push('CAVOK');
      } else {
        tokens = tokens.filter(t => t !== 'NSC' || hasSigCloud);
      }
    }
    ...
    tokens = uniq(tokens);
    const drop = !ctx.base && onlyHighClouds;
    return {
      body:tokens.join(' ').replace(/\s+/g, ' ').trim(),
      drop,
      meta:{hadCavok, had9999, hadNsw, hadNsc, hadWx, hadMetPayload}
    };
  }

  function updateState(state, body, kind) {
    const next = {...state};
  ...
  function normalize(raw) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const before = (p.body.match(/\b(?:FEW|SCT|BKN|OVC)\d{3}(?!CB\b|TCU\b)\b/g) || []).filter(t => cloud(t)?.ft >= SIG_CLOUD_FT).length;
      removedHigh += before;
      const prevCavok = state.vis9999 && !state.wx && state.noSigCloud;
      const r = normalizeBody(p.body, {base:p.kind === 'BASE', prevCavok, prevVis9999:state.vis9999});
      if (r.drop || (!r.body && p.kind !== 'BASE')) continue;
      const line = (p.prefix + r.body).replace(/\s+/g, ' ').trim();
      out.push(line);
      if (p.kind === 'BASE' || p.kind === 'BECMG' || p.kind === 'FM') state = updateState(state, r.body, p.kind);
    }
