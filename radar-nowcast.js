...
(() => {
  ...
  async function runNowcast(force=false){
    ...
    try{
      ...
      $n('rnModels').textContent=models.total?`${models.support}/${models.total} ${models.label}`:'brak danych';$n('rnModelsSub').textContent=models.rows.map(x=>`${x.name} ${x.rain.toFixed(1)} mm/h`).join(' · ')||'AIFS / GFS / ICON';$n('rnCombined').textContent=combined;
