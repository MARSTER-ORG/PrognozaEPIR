...
// Production HTML exposes Engine 2.4; this file remains the formal 2.3 Instruction kernel regression suite.
...
assert.ok(html.includes('taf-engine-v2.js?v=2.3.0-kernel'));assert.ok(html.includes('taf-engine-v24.js?v=2.4.0'));assert.ok(!/<script\s+[^>]*src=["']taf-app-v2\.js/i.test(html),'taf-app-v2 must not be active');assert.ok(html.includes("await loadScript('taf-app-v25.js?v="),'taf-app-v25 must be loaded by the active bootstrap');assert.ok(html.includes('Chmury (m AGL)'));assert.ok(html.includes('Pułap BKN/OVC (m AGL)'));
assert.ok(app.includes('profile:Array.isArray(z.profile)'),'app must transfer vertical cloud profile into TAF engine');assert.ok(app.includes('newestAtOrBefore'),'app must select an issue-time-safe observation anchor');assert.ok(app.includes('observation:anchorObservation'),'app must pass the issue-time anchor, not the latest future METAR');assert.ok(app.includes('cloudLayers'),'table must render engine cloud layers');assert.ok(app.includes('fgOperationalScore'),'FOG operational score must be separated from TAF probability');assert.ok(app.includes("api.ENGINE_VERSION!=='2.4.0'"),'app must require the public 2.4 engine');assert.ok(app.includes('await api.ready()'),'app must await local calibration data before generation');assert.ok(!app.includes('const fgRisk=Math.max(fogScore'),'raw FOG score must not be treated as literal percentage');
console.log('TAF Engine 2.3 formal kernel tests: OK');
