...
assert.equal(E.ENGINE_VERSION,'2.1.0');
...
const W=E.helpers.windToken;assert.equal(W({windKt:.4,windDir:220,gustKt:5,dirSpreadDeg:0}),'00000KT');assert.equal(W({windKt:13,windDir:274,gustKt:23,dirSpreadDeg:0}),'27014G24KT');assert.equal(W({windKt:2,windDir:220,gustKt:2,dirSpreadDeg:10}),'22002KT');
...
assert.ok(html.includes('taf-engine-v2.js?v=2.1.0'));assert.ok(html.includes('taf-app-v2.js?v=2.1.0'));assert.ok(html.includes('Chmury (m AGL)'));assert.ok(html.includes('Pułap BKN/OVC (m AGL)'));
assert.ok(app.includes('profile:Array.isArray(z.profile)'),'app must transfer vertical cloud profile into TAF engine');assert.ok(app.includes('cloudLayers'),'table must render engine cloud layers');assert.ok(app.includes("fgRisk:num(f?.vis1000)"),'fog engine FG probability must be passed separately');
console.log('TAF Engine 2.1 tests: OK');
